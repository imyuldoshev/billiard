...
function renderHistory() {
  ...
  if (todayHistory.length === 0) {
    historyWrap.innerHTML = '<div class="history-empty">Bugun hali o\'yinlar yo\'q.</div>';
    return;
  }
...
function tick() {
  ...
  state.tables.forEach(t => {
    if (t.occupied && t.startTime) {
      ...
      if (timerEl) {
        if (t.isPaused) {
          timerEl.innerHTML = `<span style="color:var(--yellow);">${formatDuration(durationMs)}</span>`;
        } else {
