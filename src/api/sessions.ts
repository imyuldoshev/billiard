...
export async function saveSessionToSupabase(session: Session) {
  ...
  const { error } = await supabase.from("table_sessions").insert({
    ...
    started_at: new Date(isNaN(Number(session.startedAt)) ? session.startedAt : Number(session.startedAt)).toISOString(),
    ended_at: new Date(isNaN(Number(session.endedAt)) ? session.endedAt : Number(session.endedAt)).toISOString(),
...
export async function loadSessionsFromSupabase(onComplete?: () => void) {
  ...
  state.history = [...remoteHistory, ...localOnlyHistory].sort(
